...
class wpiExploreTopics {
    ...
    public function __construct() {
        ...
        $this->taxo=esc_attr($wpi_explore_topics_options['wpi_taxo_field']);
    }
...
";
        if($this->taxo == NULL) $this->taxo = "tags";
...
";
        ...
        <script>
            jQuery(document).ready(function() {
                jQuery.post("<?php echo plugin_dir_url( __FILE__ ) ?>js/tags.php", {taxo: "<?php echo $this->taxo; ?>"},  function(data) {
                    if (data.length > 0) {
            ...
            function wpi_et_lookup(tags) {
                if (tags.length > 2) {
                    ...
                    jQuery.post("<?php echo plugin_dir_url( __FILE__ ) ?>js/tags.php", {taxo: "<?php echo $this->taxo; ?>", queryString: "" + tags + ""}, function(data) {
                        if (data.length > 0) {
                            jQuery('#wpi_topic_suggestions').html(data);
                        }
                    });
